...
import { html, render } from 'voby/via'
import * as Voby from 'voby/via'
import type { JSX, Observable } from 'voby/via'
import { Dynamic, ErrorBoundary, For, If, Portal, Suspense, Switch, Ternary } from 'voby/via'
import { useContext, useEffect, useInterval, useMemo, usePromise, useResource, useTimeout } from 'voby/via'
import { $, batch, createContext, createDirective, lazy, store, template } from 'voby/via'
import type { Child } from 'voby/via/types'
...
const TestSnapshots = ({ Component, props }: { Component: (JSX.Component /* | Component<any> */) & { test: { static?: boolean, snapshots: string[] }, name: string }, props?: Record<any, any> }): JSX.Element => {
    ...
    const getHTML = async (): Promise<string> => {
        const element = ref()
        if (!element) return ''
        return await get(element.innerHTML)
    }
    ...
    const tick = async () => {
        ...
        const actualHTML = await getHTML()
...
const TestSymbolRemoval = (): JSX.Element => {
    const o = $<symbol | null>(sym)
    const randomize = () => o(prev => prev ? null : sym)
    useInterval(randomize, TEST_INTERVAL)
    return (
        <>
...
TestSelectStaticOption.test = {
    static: true,
    snapshots: [
        '<select name="select-static-option"><option value="foo">foo</option><option value="bar" selected="">bar</option><option value="baz">baz</option><option value="qux">qux</option></select>'
    ]
}
...
TestSelectStaticValue.test = {
    static: true,
    snapshots: [
        '<select name="select-static-value" value="bar"><option value="foo">foo</option><option value="bar">bar</option><option value="baz">baz</option><option value="qux">qux</option></select>'
    ]
}
...
const TestSelectObservableOption = (): JSX.Element => {
    const ref = $<HTMLSelectElement>()
    const branch = $(true)
    const assert = async () => console.assert(await get(ref()?.value) === (branch() ? 'bar' : 'qux'))
...
TestSelectObservableOption.test = {
    static: true,
    snapshots: [
        '<select name="select-observable-option"><option value="foo">foo</option><option value="bar" selected="">bar</option><option value="baz">baz</option><option value="qux">qux</option></select>'
    ]
}
...
const TestSelectObservableValue = (): JSX.Element => {
    const ref = $<HTMLSelectElement>()
    const value = $('bar')
    const assert = async () => console.assert(await get(ref()?.value) === value())
...
TestSelectObservableValue.test = {
    static: true,
    snapshots: [
        '<select name="select-observable-value" value="bar"><option value="foo">foo</option><option value="bar">bar</option><option value="baz">baz</option><option value="qux">qux</option></select>'
    ]
}
...
const TestEventClickStopPropagation = (): JSX.Element => {
    ...
    return (
        <>
            ...
            <p><button onClick={incrementOuter}><div>{outer}</div><button onClick={incrementInner}>{inner}</button></button></p>
        </>
    )
}
...
const TestEventEnterStopPropagation = (): JSX.Element => {
    ...
    return (
        <>
            ...
            <p><button onMouseEnter={incrementOuter}><div>{outer}</div><button onMouseEnter={incrementInner}>{inner}</button></button></p>
        </>
    )
}
...
const TestABCD = (): JSX.Element => {
    const AB = (): JSX.Element => {
        ...
        // return <div>{component}</div> //wrap into div in order to fix the parent
        return component
    }
    const CD = (): JSX.Element => {
        const c = <b>c</b>
        const d = 'd' //<span>d</span>
        const component = $(c)
    ...
    const Page2 = () => {
        ...
        return (
            <div>
                ...
                {/* {bool ? <p>page2 - true</p> : null} */}
                <If when={() => !bool()}>
                    <p>page2 - false</p>
                </If>
                {/* {!bool ? <p>page2 - false</p> : null} */}
...
const TestTernaryObservableChildren = (): JSX.Element => {
    ...
    return (
        <>
            ...
            <div>{/** Ternary have to be bound, if not, not replacable, see TestABCD*/}
                <Ternary when={o}>
                    <AB />
                    <CD />
                </Ternary>
...
TestForRandom.test = {
    static: false,
    snapshots: [
        '<div><p>Value: {random}</p><p>Value: {random}</p><p>Value: {random}</p></div>'
    ]
}
...
const TestRefUnmounting = (): JSX.Element => {
    ...
    useEffect(async () => {
        const element = ref()
        if (element) {
            message(`Got ref - Has parent: ${!!element.parentElement /** always true since is proxy*/} - Is connected: ${await get(element.isConnected)}`)
        } else {
            message(`No ref`)
        }
    })
...
const TestRefContext = (): JSX.Element => {
    ...
    const Reffed = (): JSX.Element => {
        //not intended to fix the await
        const ref = (element) => message(`Got ref - Has parent: ${!!element.parentElement /* always true, proxied */} - Is connected: ${/* await */ get(element.isConnected)} - Context: ${useContext(Context)}`)
...
TestRefContext.test = {
    static: true,
    snapshots: [
        '<p>Got ref - Has parent: true - Is connected: [object Promise] - Context: 321</p>'
    ]
}
...
const Testrender = async (): Promise<void> => {
    ...
    const expected = '<div><h3>render</h3><p>123</p></div>'
    const actual = await render(<App />)
    assert(actual === expected, `[Testrender]: Expected '${actual}' to be equal to '${expected}'`)
}

const TestrenderSuspense = async (): Promise<void> => {
    const App = (): JSX.Element => {
        const o = $(0)
        const Content = () => {
            useResource(() => {
        ...
        return (
            <div>
                <h3>render - Suspense</h3>
                <Content />
            </div>
        )
    }
    const expected = '<div><h3>render - Suspense</h3><p>123</p></div>'
    const actual = await render(<App />)
    assert(actual === expected, `[TestrenderSuspense]: Expected '${actual}' to be equal to '${expected}'`)
}
...
TestNestedArrays.test = {
    ...
    snapshots: [
        ...
        '<button>Increment</button><ul><div></div><li>0</li><div><li>test</li></div><li>1</li><div></div><li>2</li></ul>',
